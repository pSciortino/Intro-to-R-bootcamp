...
pacman::p_load("tidyverse", "data.table", "knitr", "roxygen2", "DT", "htmlwidgets", "readxl",
               "xaringan","tufte", "fivethirtyeight","socviz","infer","tinytex", install=TRUE)
```
Note the *install=TRUE* argument which installs the packages that are not already installed and just load them otherwise.

## Code chunks
Code chunks is where you write your R code. The following options are added to define what's to be shown:

- **echo** specifies whether or not the code in the chunk is shown
- **evaluate** defines whether the code runs 
- **include** defines whether it is included that code is taken into account at all
- **warning** define whether warnings are displaying or not (by default they are!)
- **results='hide'** to hide the code's output
...
```{r}
library(DT)
datatable(tabledata, options = list(pageLength = 5))
```
...
**seq()** and **rep()** for generating vectors:
```{r}
a <- seq(from=1,to=10,by=2)
b <- rep(1,10)
```
Use **length(x)** to get the number of elements in the vector x and **nvarchar(x)** to count the # of characters foreach element in x .

```{r}
x <- c("hello","goodbye",NA)
nchar(x) # returns  5 7
(x <- x[nchar(x)==5])
"goodbye" %in% x # returns FALSE but NA is still contained in x
(x[nchar(x)==5 & (!is.na(x))]) # now only contains "hello"
```

### R objects classes {.tabset}

An **atomic vector** can be composed of elements from the same data type. A **dataframe** can only contain atomic vectors of the same length. A **list** can contain anything. **Tibbles** then are the didyverse version of dataframes (they share a lot of properties).
...
#### Tibbles
Tibbles are the tidyverse dataframes. The way they should be subseted is the same.
...
**gather(** *data*, *columns names*,key = "*New name*", value = "*name of variable containing values*"**)**
```{r}
library("knitr")
knitr::kable(table4a)
table4a %>% tidyr::gather("1999","2000",key="year",value="nrcases") %>% kable()
```
...
```{r}
ncbikecrash %>%
  sample_frac(0.2, replace=F) %>% # use sample_frac(0.2, replace=F) to get 20% of the data
  head()
```

### distinct()

The equivalent to the SQL SELECT DISTINCT county FROM ncbikecrash is:
```{r}
ncbikecrash %>%
  distinct(county) 
```
...
### count() V1
The equivalent to SELECT COUNT(*) as [Number of cases] FROM ncbikecrash GROUP BY driver_alcohol_drugs
```{r}
ncbikecrash %>%
